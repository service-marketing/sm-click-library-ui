...
const leaveGroup = inject("leaveGroup");
const removeGroupParticipant = inject("removeGroupParticipant");
const closeSelectedChat = inject("closeSelectedChat");

const handleEditGroup = () => {
  if (typeof openCreateOrEdit !== "function") return;
  openCreateOrEdit("edit", props.selectedAttendant);
};
...
const handleRemoveParticipant = async (participant) => {
  ...
  if (typeof removeGroupParticipant !== "function") return;
...
const handleLeaveGroup = async () => {
  ...
  if (!channelId) return;
  if (typeof leaveGroup !== "function") return;

  const hasLeftGroup = await leaveGroup(channelId);
  if (!hasLeftGroup) {
  ...
  if (typeof closeSelectedChat === "function") {
    closeSelectedChat();
  }
  loadingLeaveGroup.value = false;
  closeLeaveConfirmation();
  ...
  >
    ...
    <div class="flex w-full shrink-0 flex-col items-center gap-1.5">
      ...
      <h3 class="text-sm font-sans text-gray-500">
        Criado por OcultDay, 10/09/1998
      </h3>
    </div>
