...
function selectAttendant(attendant) {
    const index = attendanceSelected.value.findIndex(a => a.id === attendant.id);
    if (index !== -1) {
        attendant.selected = false;
    ...
    <div class="depart-select-container">
        ...
        <div class="search-container relative shadow shadow-gray-900 dark:shadow-gray-500">
            <div class="input-wrapper bg-base-300 border-b border-base-200" @click="open_select = !open_select"
                ...
                <input v-model="searchInput" :placeholder="attendanceSelected.length > 0
                    ? (multiSelect
            ...
            <main v-if="attendanceSelected.length > 0 && multiSelect"
                class="selection-container bg-base-300 border-b border-base-200">
                <div v-for="(attendant, index) in attendanceSelected" :key="attendant.id" class="selection-item">
                    {{ attendant.name }}
            ...
            <div :class="attendanceSelected.length > 0 && multiSelect ? 'dropdown-expanded' : 'dropdown'"
                class="dropdown bg-base-300">
                <div class="department-list">
                    <div :class="{ 'two-columns': filteredAttendants.length > 4 }" class="grid-container">
                        <div v-for="attendant in filteredAttendants" :key="attendant.id"
                            :class="{ 'selected': attendant.selected }"
                            class="department-item line-clamp-1 bg-slate-500/20 hover:bg-teal-600">
                            <div style="padding-left: 5px;">
                                <img v-if="attendant.photo" :src="attendant.photo"
                                    style="pointer-events: none; border-radius: 100%; width: 24px; height: 24px;" />
                                <svg v-else
                                    style="pointer-events: none; border-radius: 100%; width: 24px; height: 24px;"
                                    class="bg-base-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20"
                            ...
                            <span @click="selectAttendant(attendant)" class="department-name h-full w-full">{{
                                attendant.name }}</span>
                        </div>
                    </div>
                </div>
                <div v-if="!get_loading && filteredAttendants.length === 0" class="no-departments bg-base-300">
