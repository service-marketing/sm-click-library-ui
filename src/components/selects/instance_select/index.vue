...
const props = defineProps({
    token: {
        type: String,
        default: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNzIxNDA5OTM4LCJpYXQiOjE3MjEzMjM1MzgsImp0aSI6ImFlZTM1MDZjNzQ3MTQwMzRiMTE1YjJhYWNmNDkxODQ1IiwidXNlcl9pZCI6IjYyNGRmZTIzLTkyZDAtNDNmMS04MDYxLWEyMzIxZjJiNWViYiJ9.bixn8gC1aVw8XF77oR6E1X6ylM3mvhImZnAz7zahqiI'
    },
    type: String,
    webhooks: Boolean,
    client: {
...
async function getInstances() {
    try {
        let base_url = props.environment === 'development' ? 'http://localhost:8000' : props.environment === 'production' ? 'https://back.smclick.com.br' : 'https://back.homol.smclick.com.br';
        let url = '/v1/api/instances/instance/'
        let status_url = '/v1/api/instances/status_instance/'
        let params = `?client=${props.client}`;
        if (props.type) params += `&type=${props.type}`;
        if (props.webhooks) params += `&webhooks=${webhooks}`;
        ...
        instances.value.forEach((key) => {
            key.isLoading = true;
            try {
                const statusPromises = axios.get(
                    `${base_url}${status_url}?instance=${key.id}`
                    , { headers: { 'Authorization': `Bearer ${props.token}` } })
                if (statusPromises.data.instance_status !== "Disconnected") {
                    key.status = true;
                } else {
                    key.status = false;
                }
...
<template>
    <main class="w-full relative text-white">
        <div @click="open = !open" class="rounded-lg shadow shadow-gray-900 bg-base-300 p-2 px-4 text-center">
            <div class="flex cursor-pointer justify-between items-center">
                <p class="w-full select-none">
                    <span v-if="!selectedInstance">{{ instances ? instances.length : 'Sem' }} Instâncias disponíveis</span>
        ...
        <nav v-if="open"
            class="rounded-lg absolute top-10 w-full z-20 text-sm mt-2 shadow shadow-gray-900 bg-base-300 p-2">
            <ul class="gap-2 flex flex-col">
                <li v-if="instances.length >0 " v-for="inst, index in instances"
                    :class="selectedInstance && selectedInstance.id === inst.id ? 'bg-base-100' : 'bg-base-100/50'"
                    class="hover:bg-base-200/90 cursor-pointer w-full  rounded-lg">
                    <main @click="selectedInstance = inst, open = false"
                        ...
                        <div class="flex w-full items-center px-2 gap-3 justify-between">
                            ...
                            <div v-else>     <svg aria-hidden="true" class="inline w-4 h-4 text-blue-300 animate-spin  fill-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
