...
<template>
  <section class="contact-section contact-section--result">
    ...
    <div v-if="connectionsMerge.length" 
    ...
    >
      <div class="array-changes-header">
        ...
        <span v-if="connectionsNewCount" class="array-changes-count">
          +{{ connectionsNewCount }} incorporada{{ connectionsNewCount > 1 ? "s" : "" }}
        </span>
        <svg
      ...
      <div v-show="showConnections" class="connections-result-list">
        ...
        >
          ...
          <div class="conn-side conn-side--right">
            <span v-if="conn.fromChild" class="incorporated-tag">incorporado</span>
            <span v-if="conn.userName" class="conn-username">{{ conn.userName }}</span>
          </div>
        </div>
      </div>
    </div>
