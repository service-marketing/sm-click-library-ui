...
const isPopoverOpen = computed(
  () => showWaPopover.value || showIgPopover.value,
);
...
const hasContactBindings = useHasContactBindings(
  whatsappBindings,
  instagramBindings,
);
...
<template>
  ...
  >
    ...
    <div :class="infoClasses">
      ...
      <template v-else> -->
      <div
        v-if="hasPhone || hasInstagram || hasContactBindings"
      ...
      >
        ...
        <div v-if="!isSidebarMode && hasContactBindings" class="conn-btns">
          ...
          >
            <button
              type="button"
              class="conn-btn conn-btn--wa"
              @click="toggleWaPopover"
            >
              <svg
                class="size-6"
                xmlns="http://www.w3.org/2000/svg"
            ...
            <template #content>
              <div class="stack-popover" @click.stop>
                <p class="stack-popover__title stack-popover__title--wa">
                  WhatsApp
                </p>
                <div class="stack-popover__list">
                  <div
                    ...
                    :key="item.id"
                    class="stack-popover__item stack-popover__item--wa"
                  >
                    <span class="stack-popover__name">{{
                      item.instanceName
                    }}</span>
                    <span
                      v-if="item.user_name"
                      class="stack-popover__username"
                      >{{ item.user_name }}</span
                    >
                  </div>
                </div>
          ...
          >
            <button
              type="button"
              class="conn-btn conn-btn--ig"
              @click="toggleIgPopover"
            >
              ...
              <span class="conn-btn__count">{{
                instagramBindings.length
              }}</span>
            </button>
            <template #content>
              <div class="stack-popover" @click.stop>
                <p class="stack-popover__title stack-popover__title--ig">
                  Instagram
                </p>
                <div class="stack-popover__list">
                  <div
                    ...
                    :key="item.id"
                    class="stack-popover__item stack-popover__item--ig"
                  >
                    <span class="stack-popover__name">{{
                      item.instanceName
                    }}</span>
                    <span v-if="item.user_name" class="stack-popover__username"
                      >@{{ item.user_name }}</span
                    >
                  </div>
                </div>
...
.stack-popover__item--wa {
  border-left-color: rgba(52, 211, 153, 0.5);
}
.stack-popover__item--ig {
  border-left-color: rgba(221, 42, 123, 0.5);
}
