...
const gradientId = computed(
  () =>
    `ig-grad-${props.contact?.id || Math.random().toString(36).slice(2, 9)}`,
);
...
<template>
  ...
  >
    ...
    <div :class="infoClasses">
      ...
      <div v-if="hasPhone || hasInstagram" class="contact-inline-meta">
        ...
        <a
          ...
          :href="`https://www.instagram.com/${contact.instagram_user_name}/`"
          class="contact-instagram group rt-transition"
        >
          <svg
            class="size-3 opacity-70 group-hover:opacity-100 instance-ig"
            viewBox="0 0 16 16"
          >
            <defs>
              <linearGradient
          ...
          <span class="contact-instagram-user group-hover:underline">
            @{{ contact.instagram_user_name }}</span
          >
        </a>
      </div>
    ...
    >
      ...
      >
        <svg class="size-4 fill-yellow-400" viewBox="0 0 20 20">
          <path
...
.contact-instagram:hover::before {
  opacity: 0.85;
  animation: instagram-shine 900ms ease forwards;
}
